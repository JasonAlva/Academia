...
async def create_teacher(teacher: TeacherCreate, current_user: str = Depends(get_current_user)):
    teacher_service = TeacherService(prisma)
    return await teacher_service.create_teacher(teacher)

@router.get("/{teacher_id}", response_model=TeacherResponse)
async def get_teacher(teacher_id: str, current_user: str = Depends(get_current_user)):
    teacher_service = TeacherService(prisma)
...
async def update_teacher(teacher_id: str, teacher: TeacherUpdate, current_user: str = Depends(get_current_user)):
    teacher_service = TeacherService(prisma)
    try:
...
async def delete_teacher(teacher_id: str, current_user: str = Depends(get_current_user)):
    teacher_service = TeacherService(prisma)
    try:
